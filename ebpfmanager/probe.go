...
func (p *Probe) checkField() error {
	if p.EbpfFuncName == "" || p.Section == "" {
		return fmt.Errorf("EbpfFuncName:%s, Section:%s cant be null.", p.EbpfFuncName, p.Section)
	}
	...
	if p.AttachToFuncName == "" {
		return fmt.Errorf("AttachToFuncName:%s cant be null.", p.AttachToFuncName)
	}
	return nil
}
...
func (p *Probe) init() error {
	...
	if p.manualLoadNeeded {
		...
		if err != nil {
			p.lastError = err
			return fmt.Errorf("error:%v , couldn't load new probe %v", err, p.GetIdentificationPair())
		}
		p.program = prog
	}
	...
	if p.checkPin {
		...
		if p.PinPath != "" {
			if err := p.program.Pin(p.PinPath); err != nil {
				p.lastError = err
				return fmt.Errorf("error:%v , couldn't pin program %s at %s", err, matchFuncName, p.PinPath)
			}
		}
		p.checkPin = false
	}
	...
	if p.Ifindex == 0 && p.Ifname != "" {
		...
		if err != nil {
			p.lastError = err
			return fmt.Errorf("error:%v , couldn't find interface %v", err, p.Ifname)
		}
...
func (p *Probe) attach() error {
	...
	if err != nil {
		...
		_ = p.stop(false)
		return fmt.Errorf("error:%v , couldn't start probe %s", err, p.EbpfFuncName)
	}
	if p.LinkPinPath != "" && p.link != nil {
		err := p.link.Pin(p.LinkPinPath)
		if err != nil {
			return fmt.Errorf("error:%v , couldn't pin link %s", err, p.LinkPinPath)
		}
	}
	// update probe state
	p.state = running
...
func (p *Probe) stop(saveStopError bool) error {
	...
	return fmt.Errorf("error:%v , couldn't stop probe %s", err, p.EbpfFuncName)
}
...
func (p *Probe) attachTracepoint() error {
	...
	if err != nil {
		return fmt.Errorf("error:%v , couldn's activate tracepoint %s, matchFuncName:%s", err, p.Section, p.EbpfFuncName)
	}
	p.link = kp
	return nil
}
...
func (p *Probe) attachUprobe() error {
	...
	if err != nil {
		return fmt.Errorf("error:%v , couldn't enable uprobe %s", err, p.EbpfFuncName)
	}
...
func (p *Probe) attachCGroup() error {
	...
	if err != nil {
		return fmt.Errorf("error:%v , failed to attach probe %v to cgroup %s, attach type:%s", err, p.GetIdentificationPair(), p.CGroupPath, p.programSpec.AttachType.String())
	}
...
func (p *Probe) attachTCCLS() error {
	...
	if err != nil {
		if err.Error() != "netlink receive: file exists" {
			return fmt.Errorf("error:%v , couldn't add a \", err clsact\" qdisc to interface %v", err, p.Ifindex)
		}
	}
	...
	return fmt.Errorf("error:%v , couldn't add a %v filter to interface %v: %v", err, p.NetworkDirection, p.Ifindex, err)
}
...
func (p *Probe) detachTCCLS() error {
	...
	return fmt.Errorf("error:%v , couldn't detach TC classifier of probe %v", err, p.GetIdentificationPair())
}
...
func (p *Probe) attachXDP() error {
	...
	if err != nil {
		return fmt.Errorf("error:%v , couldn't retrieve interface %v", err, p.Ifindex)
	}
	...
	return fmt.Errorf("error:%v , couldn't attach XDP program %v to interface %v", err, p.GetIdentificationPair(), p.Ifindex)
}
...
func (p *Probe) detachXDP() error {
	...
	return fmt.Errorf("error:%v , couldn't detach XDP program %v from interface %v", err, p.GetIdentificationPair(), p.Ifindex)
}
...
func (p *Probe) attachRawTracepoint() error {
	...
	if err != nil {
		return fmt.Errorf("error:%v , couldn's activate raw_tracepoint %s, matchFuncName:%s", err, p.Section, p.EbpfFuncName)
	}
	p.link = link
	return nil
}
...
func (p *Probe) attachLsm() error {
	...
	if err != nil {
		return fmt.Errorf("error:%v , couldn's activate lsm %s, matchFuncName:%s", err, p.Section, p.EbpfFuncName)
	}
	p.link = link
	return nil
}
