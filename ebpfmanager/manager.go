...
func (ps *ProbeSelector) RunValidator(manager *Manager) error {
	...
	if !p.IsRunning() && p.Enabled {
		if p.GetLastError() != nil {
			return fmt.Errorf("error:%v, %s", p.GetLastError(), ps.String())
		}
		return nil
	}
	if !p.Enabled {
...
func (m *Manager) InitWithOptions(elf io.ReaderAt, options Options) error {
	...
	if m.options.RLimit != nil {
		...
		if err != nil {
			return fmt.Errorf("error:%v , couldn't adjust RLIMIT_MEMLOCK", err)
		}
	}
...
func (m *Manager) stop(cleanup MapCleanupType) error {
	...
	for _, perfRing := range m.PerfMaps {
		...
		if e != nil {
			e = fmt.Errorf("error:%v , perf ring reader %s couldn't gracefully shut down", perfRing.Stop(cleanup), perfRing.Name)
		}
	...
	for _, ringBuf := range m.RingbufMaps {
		...
		if e != nil {
			e = fmt.Errorf("error:%v , ring buf reader %s couldn't gracefully shut down", ringBuf.Stop(cleanup), ringBuf.Name)
		}
		err = ConcatErrors(err, e)
	}
	...
	for _, probe := range m.Probes {
		e := probe.Stop()
		if e != nil {
			e = fmt.Errorf("error:%v , program %s couldn't gracefully shut down", e, probe.EbpfFuncName)
		}
		err = ConcatErrors(err, e)
	}
	...
	for _, managerMap := range m.Maps {
		...
		if e != nil {
			e = fmt.Errorf("error:%v , couldn't gracefully close map %s", e, managerMap.Name)
		}
		err = ConcatErrors(err, e)
	}
...
func (m *Manager) CloneMap(name string, newName string, options MapOptions) (*ebpf.Map, error) {
	...
	if !exists {
		return nil, fmt.Errorf("error:%v , failed to clone maps/%s", ErrUnknownMap, name)
	}
...
func (m *Manager) ClonePerfRing(name string, newName string, options MapOptions, perfMapOptions PerfMapOptions) (*ebpf.Map, error) {
	...
	if !exists {
		return nil, fmt.Errorf("error:%v , failed to clone maps/%s: couldn't find map", ErrUnknownMap, name)
	}
...
func (m *Manager) AddHook(UID string, newProbe *Probe) error {
	...
	if !found || len(progs) == 0 {
		return fmt.Errorf("error:%v , couldn't find program %v", ErrUnknownMatchFuncName, oldID)
	}
	prog := progs[0]
	progSpecs, found, _ := m.GetProgramSpec(oldID)
	if !found || len(progSpecs) == 0 {
		return fmt.Errorf("error:%v , couldn't find programSpec %v", ErrUnknownMatchFuncSpec, oldID)
	}
	...
	if exists {
		return fmt.Errorf("error:%v , couldn't add probe %v", ErrIdentificationPairInUse, newProbe.GetIdentificationPair())
	}

	// Clone program
	clonedProg, err := prog.Clone()
	if err != nil {
		return fmt.Errorf("error:%v , couldn't clone %v", err, oldID)
	}
	...
	if err = newProbe.Init(m); err != nil {
		// clean up
		_ = newProbe.Stop()
		return fmt.Errorf("error:%v , failed to initialize new probe", err)
	}
	...
	if newProbe.PinPath != "" {
		if err = newProbe.program.Pin(newProbe.PinPath); err != nil {
			// clean up
			_ = newProbe.Stop()
			return fmt.Errorf("error:%v , couldn't pin new probe", err)
		}
	}
	...
	if err = newProbe.Attach(); err != nil {
		// clean up
		_ = newProbe.Stop()
		return fmt.Errorf("error:%v , couldn't attach new probe", err)
	}
		...
		if managerProbe.IdentificationPairMatches(oldID) {
			...
			if shouldStop {
				if err = managerProbe.Stop(); err != nil {
					return fmt.Errorf("error:%v , couldn't stop probe %v", err, oldID)
				}
			} else {
				if err = managerProbe.Detach(); err != nil {
					return fmt.Errorf("error:%v , couldn't detach probe %v", err, oldID)
				}
			}
			idToDelete = id
		}
...
func (m *Manager) CloneProgram(UID string, newProbe *Probe, constantsEditors []ConstantEditor, mapEditors map[string]*ebpf.Map) error {
	...
	for _, editor := range constantsEditors {
		if err := m.editConstant(newProbe.programSpec, editor); err != nil {
			return fmt.Errorf("error:%v , couldn't edit constant %s", err, editor.Name)
		}
	}

	// Write current maps
	if err = m.rewriteMaps(newProbe.programSpec, m.collection.Maps); err != nil {
		return fmt.Errorf("error:%v , couldn't rewrite maps in %v", err, newProbe.GetIdentificationPair())
	}
	...
	if err = newProbe.InitWithOptions(m, true, true); err != nil {
		// clean up
		_ = newProbe.Stop()
		return fmt.Errorf("error:%v , failed to initialize new probe %v", err, newProbe.GetIdentificationPair())
	}
	...
	if err = newProbe.Attach(); err != nil {
		// clean up
		_ = newProbe.Stop()
		return fmt.Errorf("error:%v , failed to attach new probe %v", err, newProbe.GetIdentificationPair())
	}
...
func (m *Manager) updateMapRoute(route MapRoute) error {
	...
	if !found {
		return fmt.Errorf("error:%v , couldn't find routing map %s", ErrUnknownMap, route.RoutingMapName)
	}
	...
	if route.Map != nil {
		...
		if !found {
			return fmt.Errorf("error:%v , couldn't find routed map %s", ErrUnknownMap, route.RoutedName)
		}
	...
	if err = routingMap.Put(route.Key, fd); err != nil {
		return fmt.Errorf("error:%v , couldn't update routing map %s", err, route.RoutingMapName)
	}
	return nil
}
...
func (m *Manager) updateTailCallRoute(route TailCallRoute) error {
	...
	if !found {
		return fmt.Errorf("error:%v , couldn't find routing map %s", ErrUnknownMap, route.ProgArrayName)
	}
	...
	if route.Program != nil {
		...
		if !found || len(progs) == 0 {
			return fmt.Errorf("error:%v , couldn't find program %v", ErrUnknownMatchFuncName, route.ProbeIdentificationPair)
		}
	...
	// Insert tail call
	if err = routingMap.Put(route.Key, fd); err != nil {
		return fmt.Errorf("error:%v , couldn't update routing map %s", err, route.ProgArrayName)
	}
	return nil
}
...
func (m *Manager) getProbeProgramSpec(matchFuncName string) (*ebpf.ProgramSpec, error) {
	...
	if !ok {
		...
		if !excluded {
			return nil, fmt.Errorf("error:%v , couldn't find program at %s", ErrUnknownMatchFuncName, matchFuncName)
		}
	}
	return spec, nil
}
...
func (m *Manager) matchSpecs() error {
	...
	for _, managerMap := range m.Maps {
		...
		if !ok {
			return fmt.Errorf("error:%v , couldn't find map at maps/%s", ErrUnknownMap, managerMap.Name)
		}
	...
	for _, perfMap := range m.PerfMaps {
		...
		if !ok {
			return fmt.Errorf("error:%v , couldn't find map at maps/%s", ErrUnknownMap, perfMap.Name)
		}
	...
	for _, constantEditor := range m.options.ConstantEditors {
		...
		for _, id := range constantEditor.ProbeIdentificationPairs {
			...
			if !found || len(programs) == 0 {
				return fmt.Errorf("error:%v , couldn't find programSpec %v", ErrUnknownMatchFuncName, id)
			}
			prog := programs[0]

			// Edit program
			if err := m.editConstant(prog, constantEditor); err != nil {
				return fmt.Errorf("error:%v , couldn't edit %s in %v", err, constantEditor.Name, id)
			}
		}
		...
		if len(constantEditor.ProbeIdentificationPairs) == 0 {
			for section, prog := range m.collectionSpec.Programs {
				if err := m.editConstant(prog, constantEditor); err != nil {
					return fmt.Errorf("error:%v , couldn't edit %s in %s", err, constantEditor.Name, section)
				}
			}
		}
	}
...
func (m *Manager) editMapSpecs() error {
	for name, mapEditor := range m.options.MapSpecEditors {
		...
		if !exists {
			return fmt.Errorf("error:%v , failed to edit maps/%s: couldn't find map", ErrUnknownMap, name)
		}
...
func (m *Manager) rewriteMaps(program *ebpf.ProgramSpec, eBPFMaps map[string]*ebpf.Map) error {
	for symbol, eBPFMap := range eBPFMaps {
		...
		if err != nil {
			return fmt.Errorf("error:%v , couldn't rewrite map %s", err, symbol)
		}
	}
	return nil
}
...
func (m *Manager) loadPinnedMap(managerMap *Map) error {
	...
	*/
	...
	if err != nil {
		return fmt.Errorf("error:%v , couldn't load map %s from %s", err, managerMap.Name, managerMap.PinPath)
	}
...
func (m *Manager) loadPinnedProgram(prog *Probe) error {
	...
	*/
	...
	if err != nil {
		return fmt.Errorf("error:%v , couldn't load program %v from %s", err, prog.GetIdentificationPair(), prog.PinPath)
	}
...
func (m *Manager) loadPinnedLink(prog *Probe) error {
	...
	if err != nil {
		return fmt.Errorf("error:%v , couldn't load link %v from %s", err, prog.GetIdentificationPair(), prog.LinkPinPath)
	}
...
func (m *Manager) sanityCheck() error {
	...
	for _, managerMap := range m.Maps {
		...
		if ok {
			return fmt.Errorf("error:%v , map %s failed the sanity check", ErrMapNameInUse, managerMap.Name)
		}
	...
	for _, perfMap := range m.PerfMaps {
		...
		if ok {
			return fmt.Errorf("error:%v , map %s failed the sanity check", ErrMapNameInUse, perfMap.Name)
		}
	...
	for _, managerProbe := range m.Probes {
		...
		if ok {
			return fmt.Errorf("error:%v , %v failed the sanity check", ErrCloneProbeRequired, managerProbe.GetIdentificationPair())
		}
...
func (m *Manager) newNetlinkConnection(ifindex int32, netns uint64) (*netlinkCacheValue, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error:%v , couldn't open a NETLink socket in namespace %v", err, netns)
	}
