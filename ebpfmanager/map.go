...
func loadNewMap(spec ebpf.MapSpec, options MapOptions) (*Map, error) {
	...
	if managerMap.PinPath != "" {
		if err := managerMap.array.Pin(managerMap.PinPath); err != nil {
			return nil, fmt.Errorf("error:%v , couldn't pin map %s at %s", err, managerMap.Name, managerMap.PinPath)
		}
	}
...
func (m *Map) Init(manager *Manager) error {
	...
	if m.array == nil {
		...
		if !ok {
			return fmt.Errorf("error:%v, couldn't find map at maps/%s", ErrUnknownSection, m.Name)
		}
		m.array = array
		...
		if m.PinPath != "" {
			if err := m.array.Pin(m.PinPath); err != nil {
				return fmt.Errorf("error:%v , couldn't pin map %s at %s", err, m.Name, m.PinPath)
			}
		}
	}
	m.state = initialized
