...
func OpenAndListSymbols(path string) (*elf.File, []elf.Symbol, error) {
	...
	f, err := elf.Open(path)
	if err != nil {
		return nil, nil, fmt.Errorf("error:%v , couldn't open elf file %s", err, path)
	}
	defer f.Close()
	...
	if len(syms) == 0 {
		var err error
		if errSyms != nil {
			err = fmt.Errorf("error:%v , failed to list symbols", err)
		}
		if errDynSyms != nil {
			err = fmt.Errorf("error:%v , failed to list dynamic symbols", err)
		}
		if err != nil {
			return nil, nil, err
		} else {
